...
SELECT SUM(TotalAmount) AS TotalSpent FROM Orders;

SELECT UserType, SUM(TotalAmount) AS TotalSpent
FROM Users
...
SELECT OrderDetails.OrderDetailID, Orders.OrderID, Products.Name, OrderDetails.Quantity, OrderDetails.Subtotal
FROM OrderDetails
JOIN Orders ON OrderDetails.OrderID = Orders.OrderID
JOIN Products ON OrderDetails.ProductID = Products.ProductID;

SELECT Patients.Name, Appointments.AppointmentDateTime
FROM Patients
JOIN Appointments ON Patients.PatientID = Appointments.PatientID
WHERE Appointments.AppointmentDateTime > '2023-05-01';

SELECT d1.Name AS Doctor1, d2.Name AS Doctor2, d1.Specialization
FROM Doctors d1
JOIN Doctors d2 ON d1.Specialization = d2.Specialization AND d1.DoctorID < d2.DoctorID;

SELECT Patients.Name, Appointments.AppointmentDateTime, Appointments.Status
FROM Patients
...
SELECT Patients.Name AS PatientName, Doctors.Name AS DoctorName, Appointments.AppointmentDateTime, Appointments.Status
FROM Patients
JOIN Appointments ON Patients.PatientID = Appointments.PatientID
JOIN Doctors ON Appointments.DoctorID = Doctors.DoctorID;

CREATE VIEW PatientOrderSummary AS
SELECT Orders.OrderID, Patients.Name AS PatientName, Orders.OrderDate, Orders.TotalAmount, GROUP_CONCAT(Products.Name SEPARATOR ', ') AS ProductsOrdered
FROM Orders
...
select * from PatientOrderSummary;

-- Trigger for BEFORE INSERT on Orders
-- Trigger for BEFORE INSERT on Orders
DELIMITER $$
CREATE TRIGGER check_order_total_insert BEFORE INSERT ON Orders FOR EACH ROW
BEGIN
...
-- Trigger for BEFORE UPDATE on Orders
DELIMITER $$
CREATE TRIGGER check_order_total_update BEFORE UPDATE ON Orders FOR EACH ROW
BEGIN
    IF NEW.TotalAmount < 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Order total must be non-negative';
    END IF;
...
-- Example: Create a trigger to ensure that a doctor cannot be deleted if they have appointments
CREATE TRIGGER prevent_delete_doctor_with_appointments
...
BEGIN
    ...
    IF appointment_count > 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cannot delete a doctor with appointments';
    END IF;
END;
DELIMITER;

-- Example: Create an audit trail trigger for the Orders table
DELIMITER $$
CREATE TRIGGER audit_orders
AFTER INSERT ON Orders
FOR EACH ROW
BEGIN
    INSERT INTO AuditTrail (TableName, Action, ActionTimestamp)
    VALUES ('Orders', 'Inserted', NOW());
END$$
